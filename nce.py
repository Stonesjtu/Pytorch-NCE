...
class NCELoss(nn.Module):
    ...
    """
    def __init__(self, noise, noise_ratio, norm_term, ntokens, size_average=True):
    ...
    def forward(self, input, target):
        ...
        noise_samples = torch.LongTensor(randint(self.ntokens - 1, size=(target.size(0), self.noise_ratio))) + 1
        ...
        noise_loss = torch.sum(
            (self.noise_ratio * noise_probs) / (noise_in_data_probs + self.noise_ratio * noise_probs)
        , 1)
